...
pub fn reinitialize(task: &mut task::Task) {
    *task.save_mut() = SavedState::default();
        ...
        })
    {
        ...
        let stack_slice = (initial_stack - frame_size)
            .checked_sub(region.base as usize)
            .filter(|_| okay)
            .and_then(|region_size| {
                USlice::<u32>::from_raw(region.base as usize, region_size >> 2).ok()
            });
        if let Some(mut uslice) = stack_slice {
